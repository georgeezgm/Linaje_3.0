...
#FIN FUNCIONES----
  ...
  ## VARIABLES DE NIVELES PROFUNDIDAD Y OUTPUT O INPUT EN SALIDA DE DATOS Y----



  ## POSICION EN RUTAS DATOS SALIDA----
...
rm(tabla_analisis)
    ...
    grupo_tablas_analisis<-c("dwbasep0.dwipdltt")
   xvalor<-"dwbasep0.dwipdltt"
   x_val<-5
   x<-x_val
   ni_x<-x_val
   y_val<-c("out")
...
# }
  ...
  {
    if (length(V(dt[[i]]))>0 & length(which(V(dt[[i]])$name== tabla_analisis))>0) {
      comp_conxa_tabla_t<- i; print(i)}
  }
  ## BUCBLE X e Y----
for(y_val in y)
  {
...
#grupos_teradata<-c("Tabla de teradata","Tablas de Producción Teradata",
#"Tablas de Arquitectura",
#"Tablas de Proyectos",
#"Tablas de TELCO","Tablas de dato",
#"Tablas de usuario","Tablas de proceso")
#grupos_exadata<-c("Tabla de exadata","Tablas de Producción Exadata")
  ...
  ## NOMBRE DEL GRAFO----
      ...
      print(name_igraph)
      print(name_image_igraph)
      print("Empezamos igraph")
### IGRAPH----
igraph<-visIgraph(
            t_arbol_m_out, 
            physics=TRUE, 
...
#    visClusteringByGroup(groups="Cluster BDA pro", label="Cluster: ",shape="box",color=c(background="palegreen3") %>% 
  ...
  #  width=0.2, 
  #  position="left" , 
  #  main=list(text= "AGRUPACIONES DE BASES DE DATOS",
#style="font-family:Times New Roman;serif;font-weight:bold;font-size:14px;text-align:center"),
  #  zoom=FALSE
  #) %>% 
              #FIN ESCENAS ELIMINADAS----
            visLayout(
...
## SAVE IGRAPH----
  ...
  visSave( 
              igraph,file=paste0(rutacarpeta,"/",
            str_to_upper(sub(".","_",tabla_analisis,
                             ignore.case=FALSE, 
                             perl=FALSE,
                             fixed=TRUE, 
                             useBytes=FALSE)),
                          "_",x_val,"_",y_val,".html")
         )
  ...
  image_igraph[["x"]][["byselection"]][["enabled"]]=FALSE
      ...
      #  plot(image_igraph)
      # dev.off()
      #visSave(image_igraph,file=paste0(rutacarpeta,"/",
      # str_to_upper(sub(".","_",tabla_analisis,ignore.case=FALSE, 
...
#visSave(igraph,file=paste0(rutacarpeta,"/",str_to_upper(sub(".","_",tabla_analisis,ignore.case=FALSE, 
#perl=FALSE,fixed=TRUE, 
#useBytes=FALSE)),"_",x_val,"_",y_val,".html"))
