  ...
  lecturafichero<- function(n1,n2,n3,n4)
{
  ...
  if (cargar_vistas == 1){
        ...
        print("   1.3 Juntamos ficheros")
              ...
              datos_1_juntos$tabla<-as.character(paste0("1.",datos_1_juntos$tabla))
        ...
        datos_1_juntos$tabla<-as.character(paste0("1.",datos_1_juntos$tabla))
        datos_juntos<-datos_1_juntos
        vistas_cargadas<-"NO ES NECESARIO"
  }       
    
    if (cargar_tablas_exa == 1){
    ## FICHERO DE DATOS ##
    #LECTURA DE DATOS EXADATA
    datos_2_exa<- read_delim(paste0(rutadatosentrada,nombre_fichero_datos_exa), delim=separador_datos_exa,quote=quote_datos_exa, col_names = c('co_query','tabla','tipo','tabla_vista','orden'))
    names(datos_2_exa)<- c('co_query','tabla','tipo','tabla_vista','orden')
    datos_2_exa$tabla<-as.character(paste0("2.",datos_2_exa$tabla))
    datos_juntos<-rbind(datos_1_juntos,datos_2_exa)
    
    }
    
    
  attach(datos_juntos)
  datos<- datos_juntos[order(co_query,orden),]
  rm(datos_1_juntos)
  rm(datos_1_fichero)
  rm(datos_1_vistas)
  rm(datos_0_vistas)
  ...
  cadena_tablas_borrar<-"\\.err|\\.error|\\.old|carttt$|carstt$|cartvt$|carsvt$"
...
#  t<- delete_vertices(t, matrix(which(!is.na(str_match(V(t)$name, cadena_tablas_borrar)))))
  ...
  print(paste0("Total de Vertices a borrar: ",numero_vertices_borrar))
  tinicial<-t
  j<-0
