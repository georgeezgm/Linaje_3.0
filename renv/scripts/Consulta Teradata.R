...
is.function(pollfunc)
 ...
 #    l<-9
   ...
   ruta<-rutajdbc
                                ...
                                # CONEXIONES ----
                                drvvv<-JDBC("com.teradata.jdbc.TeraDriver",paste0(ruta,"/terajdbc4.jar"))
                                ...
                                DBI::dbDisconnect(C)
               #        ;res},
                #       args=list(V=l,ruta=rutajdbc),
                 #      package = TRUE));
         
rm(res)
  # assign(x_arbol(l,1,"rp"),res)
...
for (l in lx){
  if(l==0){
    rp<-rp0 
  print("cargado nivel 0")}
  if(l!=0){
    assign("rpsuma",paste0("rp",l))
    assign("rpsuma",paste0("rp",l))
    rbind(rp,rpsuma)
    print(paste0("cargado nivel ",l))
    }
