...
lecturafichero<- function(n1,n2,n3,n4){
 ...
 # datos_1_fichero<- lecturafichero(paste0(rutadatosentrada,nombre_fichero_datos),'\t','\"',c('co_query','tabla','tipo','tabla_vista','orden'))
  ...
  datos_traspasos<- read_delim(paste0(rutadatosentrada,nombre_fichero_traspasos),delim=separador_traspasos,quote=quote_traspasos,col_names = c('tabla_in','tabla_out','tipo'),skip=1)
  datos_filt_total<-bind_rows(datos_filt_tot,datos_traspasos)
