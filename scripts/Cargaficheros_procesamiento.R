 ...
 # datos_1_fichero<- lecturafichero(paste0(rutadatosentrada,nombre_fichero_datos),'\t','\"',c('co_query','tabla','tipo','tabla_vista','orden'))
  ...
  if (cargar_vistas == 1){
   ...
   # datos_0_vistas<- lecturafichero(paste0(rutadatosentrada,nombre_fichero_vistas),
    ...
    datos_1_exa<- read_delim(paste0(rutadatosentrada,nombre_fichero_datos_exa), delim=separador_datos_exa,quote=quote_datos_exa, col_names = c('co_query','tabla','tipo','tabla_vista','orden'))
    names(datos_1_fichero)<- c('co_query','tabla','tipo','tabla_vista','orden')
  ...
  datos[,2] <- unlist(mclapply(datos[,2],function(x) gsub('2020[0-9][0-9][0-9][0-9]','YYYYMMDD',x)))
  ...
  edges<- as.vector(t(datos_filt_tot[,1:2]))
  edges_lab<- as.vector(t(datos_filt_tot[,3]))
